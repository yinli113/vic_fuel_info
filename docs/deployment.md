...
- **Ingestion secrets**: In the repo → **Settings → Secrets and variables → Actions**, set `SERVO_SAVER_API_CONSUMER_ID` and **either** `POSTGRES_DB_URL` **or** the discrete `POSTGRES_HOST` / `POSTGRES_USER` / `POSTGRES_PASSWORD` (same names as Streamlit). **Important:** the app **prefers discrete** only when **all three** of host, user, and password are set. If one is missing, it **falls back to `POSTGRES_DB_URL`**. So if you moved to discrete vars in Streamlit but left an **old** `POSTGRES_DB_URL` in Actions (or forgot `POSTGRES_USER` there), ingestion can write to a **different** database than the UI — delete or replace the unused URL secret when using discrete vars. If Actions has no valid DB env, ingestion never writes and ingest days stay old.
